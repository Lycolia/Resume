...
担当: |-
    * PG として技術選定から参画
      フロントサイドでの利用技術提案として
      既存のフロントが1枚のHTMLにjQueryを埋め込んだSPAだったが、
      コンポーネント化されておらず、全体的な品質も良くはなかったため、
      その部分を改善するためにReact + TypeScriptを提案
      採用理由としては静的解析による実装の整合性担保や、型推論による実装効率の向上がある
    ...
    * コミュニケーションツールの改善提案
    * テストツールの改善提案
      サーバーサイドの手動テストがcurlであり、セッションCookieが操作できないとのことだったのでPOSTMANを提案
      設定を組めば誰でもテストができる上、Cookieの書き換えも容易なため、よりブラウザに近い人力テストが行えるようになった
使用技術:
